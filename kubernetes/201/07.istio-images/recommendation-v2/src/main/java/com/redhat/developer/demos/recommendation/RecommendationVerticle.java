...
public class RecommendationVerticle extends AbstractVerticle {
  ...
  private static final int LISTEN_ON = Integer.parseInt(System.getenv().getOrDefault("LISTEN_ON", "8080"));
  ...
  public void start() throws Exception {
    ...
    router.get("/slow").handler(this::slow);
    router.get("/fast").handler(this::fast);
  ...
  private void slow(RoutingContext ctx) {
    this.slow = true;
    logger.info("'slow' has been set to 'true'");
    ctx.response().end("Following requests to '/' will be delayed by 2 secs\n");
  }

  private void fast(RoutingContext ctx) {
    this.slow = false;
    logger.info("'slow' has been set to 'false'");
    ctx.response().end("Following requests to '/' will no longer have any delay\n");
  }
